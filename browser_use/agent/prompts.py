...
class SystemPrompt:
    def __init__(self, action_description: str, current_date: datetime):
        """
        Initialize the SystemPrompt with action description and current date.

        Args:
            action_description (str): Description of the available actions.
...
9. If popups like cookies appear, accept or close them
    ...
    def input_format(self) -> str:
        """
        Returns the input format for the agent.
...
class AgentMessagePrompt:
    def __init__(self, state: BrowserState):
        """
        Initialize the AgentMessagePrompt with the current browser state.
    ...
    def get_user_message(self) -> HumanMessage:
        """
        Get the user message based on the current browser state.

        Returns:
            HumanMessage: The user message with state description and optional screenshot.
        """
        state_description = f"""
...
{self.state.dom_items_to_string()}
    ...
    def get_message_for_history(self) -> HumanMessage:
        """
        Get the message for history based on the current browser state.

        Returns:
            HumanMessage: The message for history with the current URL.
        """
